...
type easingType = (el: Element, i: number, total: number) => any | 'linear' | 'easeInQuad'
    | 'easeInCubic' | 'easeOutQuad' | 'easeInOutQuad'
    | 'easeInQuart' | 'easeOutCubic' | 'easeInOutCubic'
    | 'easeInQuint' | 'easeOutQuart' | 'easeInOutQuart'
    | 'easeInSine' | 'easeOutQuint' | 'easeInOutQuint'
    | 'easeInExpo' | 'easeOutSine' | 'easeInOutSine'
    | 'easeInCirc' | 'easeOutExpo' | 'easeInOutExpo'
    | 'easeInBack' | 'easeOutCirc' | 'easeInOutCirc' | string
    | EasingSteps | EasingOutElastic | EasingSpring | EasingCubicBezier
 ...
 */
interface Params {
    delay?: (el, index, total) => void | number;
    duration?: (el, index, total) => void | number;
    autoplay?: boolean;
    loop?: boolean | number;
    ...
    update?: (animation) => void;
    complete?: (animation) => void;
    loopBegin?: (animation) => void;
    loopComplete?: (animation) => void;
    change?: (animation) => void;
    changeBegin?: (animation) => void;
    changeComplete?: (animation) => void;
}
...
export type AnimationParams = CSSProperties & VariantCss & Params
    & { [key: string]: VariantChangeable }
    & { [key: string]: Array<string | number> } //强制动画以指定值开始
...
function anime(params: AnimationParams = {}) {

    let startTime = 0, lastTime = 0, now = 0;
    let children, childrenLength = 0;
    let resolve = null;

    if (typeof params.easing === "object") {
        let p: any = params;
        if (p.easing["numberOfSteps"] != undefined) {
            p.easing = "steps(" + p.easing["numberOfSteps"] + ")";
        }
        if (p.easing["type"] != undefined) {
            if (p.easing["type"] == 'outElastic') {
                p.easing = "easeOutElastic(" + p.easing["amplitude"] + "," + p.easing["period"] + ")";
            }
            if (p.easing["type"] == 'inElastic') {
                p.easing = "easeInElastic(" + p.easing["amplitude"] + "," + p.easing["period"] + ")";
            }
            if (p.easing["type"] == 'inOutElastic') {
                p.easing = "easeInOutElastic(" + p.easing["amplitude"] + "," + p.easing["period"] + ")";
            }
        }
        if (p.easing['mass'] != undefined && p.easing['stiffness'] != undefined) {
            p.easing = "spring(" + p.easing['mass'] + ", " + p.easing['stiffness'] + ", " + p.easing['damping'] + ", " + p.easing['velocity'] + ")"
        }
        if (p.easing['x1'] != undefined && p.easing['y1'] != undefined) {
            p.easing = "cubicBezier(" + p.easing['x1'] + "," + p.easing['y1'] + "," + p.easing['x2'] + "," + p.easing['y2'] + ")"
        }
    }
    ...
    let instance: any = createNewInstance(params);
...
function stagger(val, params: any = {}) {
...
function timeline(params: AnimationParams = {}) {
    let tl = anime(params);
    tl.duration = 0;
    tl.add = function (instanceParams, timelineOffset) {
        ...
        let insParams: any = mergeObjects(instanceParams, replaceObjectProps(defaultTweenSettings, params));
...
export const GinkgoAnimation = anime;
